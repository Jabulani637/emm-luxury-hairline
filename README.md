...
public/                npm run pages OUTPUT — do not hand-edit the .html here
  *.html               index, cart, pages/*, one file per product and collection
  ...
  vercel.json          routing config — see the repository-root copy below
...
vercel.json            the same routing rules, at the repository root. Which of the
                       two copies Vercel reads is unresolved, so both exist and
                       npm run smoke fails if they ever differ
...
The frontend's routing config exists twice, deliberately, because which of the two
copies Vercel reads has never been settled and guessing wrong takes the shop down.
`vercel.json` at the repository root and `public/vercel.json` carry byte-identical
rules, and `npm run smoke` fails the moment they differ — which makes the open
question harmless instead of load-bearing. What those rules are:
...
The **security headers are still not applied**: the storefront sends no CSP, no
`X-Content-Type-Options`, no `X-Frame-Options` and no `Referrer-Policy`, and
nothing may be assumed to be protected by them until a live response is inspected.
An earlier revision of the root copy carried a policy that had never been checked
against anything; this is that policy, now measured:
...
Counted over the 17 committed pages and their CSS and JS: no inline executable
`<script>` (the two blocks that exist are `application/ld+json`, which CSP does not
touch), no `onclick`-style handler attributes, no `<iframe>`, no `<form action>`,
no `object` or `embed`, no `document.write`, no `eval` or `new Function`, and no
worker or `blob:` URL. Every one of the eleven script tags the live homepage
returns is same-origin `/js/…`, and Vercel injects nothing of its own into the
HTML. The only external origins referenced anywhere are `fonts.googleapis.com`
(stylesheets), `cdn.shopify.com` (product images) and `data:` URIs in
`css/components.css` — all three already named in the policy above.

Two things a file scan cannot see, and both are covered: the font *files* are
fetched by Google's stylesheet from `fonts.gstatic.com` rather than from any file
here, and Shopify's image URLs reach the page at runtime through
`public/js/product.js`.

So the blocker is no longer evidence but decision. A `headers` block in either copy
changes every response on a host that has already 404d the whole site over one
omitted key, and `script-src 'self'` fails quietly — the page still renders and its
buttons simply stop working — so this is enabled deliberately, one host at a time,
with a live response checked afterwards.
